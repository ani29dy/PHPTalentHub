...
const BusinessDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      const [jobRes, appRes, hiringRes] = await Promise.all([
        axios.get("/api/jobs/my/jobs", authConfig),
        axios.get("/api/jobs/applications/my/list", authConfig),
        axios.get("/api/profiles/business/me/hiring-requests", authConfig),
      ]);
      setJobs(jobRes.data);
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        ...
        <div className="flex gap-3">
          <Link
            to="/create-job"
            className="inline-flex items-center gap-2 bg-violet-600 hover:bg-violet-700 text-white px-5 py-2.5 rounded-xl font-bold text-sm transition-colors shadow-lg shadow-violet-200"
          >
            + Post a Job
          </Link>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <div
          className={`rounded-2xl border shadow-sm p-5 ${pendingCount > 0 ? "bg-yellow-50 border-yellow-200" : "bg-white border-slate-100"}`}
        >
          <p
            className={`text-3xl font-black ${pendingCount > 0 ? "text-yellow-700" : "text-slate-900"}`}
          >
            {pendingCount}
          </p>
          <p
            className={`text-sm mt-1 ${pendingCount > 0 ? "text-yellow-600 font-semibold" : "text-slate-500"}`}
          >
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
          ...
          <p className="text-slate-500 text-sm mt-1">Hiring Requests Sent</p>
        </div>
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4 mb-8">
        <Link
          to="/search"
          className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 card-hover flex items-center gap-4"
        >
          ...
          <div>
            ...
            <p className="text-slate-500 text-xs mt-0.5">
              Search & filter verified PHP talent
            </p>
          </div>
        </Link>
        <Link
      ...
      <div className="flex gap-1 mb-6 bg-slate-100 p-1 rounded-xl w-fit">
        {(["jobs", "applications", "hiring"] as const).map((tab) => (
          <button
            key={tab}
          ...
          >
            {tab === "jobs"
              ...
              : `Hiring Sent (${hiringRequests.length})`}
                ...
                <div className="flex items-start gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      ...
                      {job.jobType && (
                        <span
                          className={`text-xs font-semibold px-2.5 py-0.5 rounded-full capitalize ${JOB_TYPE_COLORS[job.jobType] || "bg-slate-100 text-slate-600"}`}
                        >
                          {job.jobType.replace("-", " ")}
                        </span>
                      )}
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {job.skills.slice(0, 4).map((s, i) => (
                        <span
                          key={i}
                          className="bg-violet-50 border border-violet-100 text-violet-700 text-xs font-semibold px-2.5 py-1 rounded-full"
                        >
                          {s}
                        </span>
                      ))}
          ...
          {applications.length === 0 ? (
            <div className="bg-white rounded-2xl border border-slate-100 p-16 text-center">
              ...
              <p className="text-slate-500">
                Applications from developers will appear here once they apply to
                your jobs.
              </p>
            </div>
          ) : (
                  ...
                  <div className="bg-slate-50 rounded-xl p-4 mb-4">
                    <div className="grid sm:grid-cols-3 gap-3 text-sm mb-3">
                      <div>
                        ...
                        <p className="font-semibold text-slate-800">
                          {app.developerProfile.experience}
                        </p>
                      </div>
                      <div>
                        <span className="text-slate-400 text-xs">Location</span>
                        <p className="font-semibold text-slate-800">
                          {app.developerProfile.location}
                        </p>
                      </div>
                      <div>
                        <span className="text-slate-400 text-xs">Status</span>
                        <p className="font-semibold text-emerald-600">
                          {app.developerProfile.verified
                            ? "✓ Verified"
                            : "Unverified"}
                        </p>
                      </div>
                    </div>
                    {app.developerProfile.bio && (
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {app.developerProfile.skills?.slice(0, 5).map((s, i) => (
                        <span
                          key={i}
                          className="bg-violet-50 border border-violet-100 text-violet-700 text-xs font-semibold px-2 py-0.5 rounded-full"
                        >
                          {s}
                        </span>
                      ))}
      ...
      {activeTab === "hiring" && (
        <div className="space-y-4">
          {hiringRequests.length === 0 ? (
            <div className="bg-white rounded-2xl border border-slate-100 p-16 text-center">
              <div className="text-5xl mb-4">✉</div>
              <h3 className="text-xl font-bold text-slate-900 mb-2">No hiring requests sent yet</h3>
              <p className="text-slate-500">
                When you send a hire inquiry to a developer, it will appear here.
              </p>
            </div>
          ) : (
            hiringRequests.map((req) => (
              <div key={req._id} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
                <div className="flex items-start justify-between gap-4 mb-3">
                  <div className="flex items-start gap-3">
                    <div className="w-10 h-10 rounded-full bg-violet-600 flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
                      {initials(req.toUser?.name || "D")}
                    </div>
                    <div>
                      <h3 className="font-bold text-slate-900">{req.toUser?.name}</h3>
                      <p className="text-slate-500 text-xs">{req.toUser?.email}</p>
                    </div>
                  </div>
                ...
                {req.message && (
                  <p className="text-slate-500 text-sm bg-slate-50 rounded-xl px-4 py-3 mb-3 border border-slate-100">
                    {req.message}
                  </p>
                )}
